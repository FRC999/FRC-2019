...
public class ElevatorRobot extends TimedRobot {
  ...
  public void teleopInit() {
    ...
    elevator.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Absolute);  
      ...
      double rotator_kD = 0;
      double rotator_kF = 0;
      rotator.config_kP(0, rotator_kP);
      rotator.config_kI(0, rotator_kI);
      rotator.config_kD(0, rotator_kD);
      rotator.config_kF(0, rotator_kF);
  ...
  /*  if (runRotator && !runElevator) {
    ...
    /*if (runRotator) {
      rotator.set(ControlMode.MotionMagic, rotatorSetPoint);
    } else if (rotateManual) {
      if (rotator.getSelectedSensorPosition() <= 1200) {
        rotator.set(.25);
      }
    } else {
      rotator.set(0);
 ...
 /* @Override
  public void testPeriodic() { //elevator position values: 0 to 15000
    elevatorUp = leftStick.getRawButton(5);
    elevatorDown= leftStick.getRawButton(3);
    zeroEnc = leftStick.getRawButton(11);
    ...
    elevator.set(-elevatorSpeed);
  } else if (elevatorPos < elevatorMin && (elevatorUp || elevatorDown)) {
