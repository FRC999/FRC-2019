 ...
 * (ish). Each instance builds and operates one WPI_TalonSRX for PID control.
 * Inherited by classes MagicElevator, MagicClimber, heavily based on CTRE's
 * example PID code,
 * 
...
public class MagicPIDMotor {
  ...
  /**
   *
   * @param cir        The circumference of the spool/wheel (2.54*Math.PI*2 for
   *                   the elevator)
   * @param gearRat    The ratio of the connected gearbox (input rotations/output
   *                   rotations)
   ...
   * @param slot       "Which PID slot to pull gains from. Starting 2018, you can
   *                   choose from 0,1,2 or 3. Only the first two (0,1) are
   *                   visible in web-based configuration." NOT the port number.
   * @param port       the port number of the talon that this class operates
   * @param startPoint the starting point of the system. Ie, if the system starts
   *                   at 90 ticks positive of it's intended zero, set to 90.
   * @param mi         the minimum point (for validation)
   * @param ma         the maximum point (for validation)
   */
  public MagicPIDMotor(double cir, double gearRat, double P, double I, double D, double F, double peakOutput, int slot,
      int port, int startPoint, int mi, int ma) {
   ...
   * 
   * @param input Centimeter height of the elevator (off the ground? not? more
   *              testing!)
  ...
  /**
   * Converts native units to centimeters, for logging and puting back in
   * MagicOutput.
   * 
   * @param input native talon units for conversion
   * @return centimer height of elevator (off the ground?), iff I got my math
   *         right
   */
   ...
   * 
   * @param newPos the setpoint "in encoder ticks or an analog value, depending on
   *               the sensor", according to CTRE's documentation on the
   *               WPI_TalonSRX
   */
  ...
  /**
   * Moves talon to the selected position If it is at the target position,
   */
  ...
  /**
   * Change P, I, D, F to preferred values, for testing Comment out in
   * commpetition builds
   */
  ...
  /**
   * Increase PIDF values by the inputted numbers Input negative values for
   * decreasing Intended for tuning: Remove before release
   */
  ...
  /**
   * Prints tuning values User must invoke printMagicLine to print
   */
