 ...
 * It is primarily focused on the joysticks. It provides this information to
 * other classes and methods, free of charge. MagicInput, unlike other
 * off-the-shelf input brands, is designed for configuration, first and
 * foremost. Where is button is, and its toggling behavior, is specified in
 * ButtonEnum. Do not use MagicInput if you like writing long lines of code any
 * time you wish to speak with the manager.
 *
 * Use getJoystick() to fetch joysticks if you want to throw together a button
 * without putting it in ButtonEnum. Otherwise, keep buttons there.
 */
public class MagicJoystickInput {

  private static MagicJoystickInput mInstance = new MagicJoystickInput();

  public static MagicJoystickInput getInstance() {
    return mInstance;
  }
  ...
  /**
   * Note: Code checks if joystick is null: however, this (should) never be the
   * case
   */
  ...
  /**
   * Checks whether a button is "on" ie, it is toggled on or pressed. If it is a
   * toggling button, checks whether it is toggled on, otherwise, checks if it is
   * pressed
   * 
  ...
  /**
   * Checks whether the physical button is currently being held down. Does *not*
   * account for TogglingButtons: only use if you are *certain* it will never be
   * one
   * 
   * @param type is a ButtonEnum button. Format it like ButtonEnum.selectedButton
   * @return true if on: false if not
  ...
  /**
   * Gets how far left or right the turn stick is. "Left is positive"--Jack Wertz
   * 2019
   * 
  ...
  /**
   * Gets the elevator axis: we haven't decided how to ultimately treat this We
   * should probably annoy the drive team or something
   * 
  ...
  /**
   * Updates TogglingButtons in ButtonEnum This *must* be called every cycle for
   * TogglingButtons to function It also updates elevatorHeight with the current
   * selection If more than one elevator height selector button is pressed, it
   * should go to the last one on the ButtonEnum list. Should.
   */
  void updates() {
